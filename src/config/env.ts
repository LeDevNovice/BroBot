...
export interface AppConfig {
  ...
  readonly DASHBOARD_CHANNEL_ID?: string;
}
...
class ConfigValidator {
  ...
  private static parseAuthorizedUsers(): readonly string[] {
    ...
    if (!usersEnv) {
      console.warn(
        "No authorized users configured. Bot will be accessible to no one.",
      );
      return [];
    }
    ...
    if (users.length === 0) {
      console.warn("No valid authorized users found.");
    }

    const invalidUsers = users.filter((id) => !/^\d+$/.test(id));
    if (invalidUsers.length > 0) {
      throw new Error(
        `Invalid Discord IDs in AUTHORIZED_USERS: ${invalidUsers.join(", ")}`,
      );
    }
  ...
  private static parsePort(): number {
    ...
    if (isNaN(port) || port < 1 || port > 65535) {
      throw new Error(
        `Invalid PORT value: ${portEnv}. Must be a number between 1 and 65535.`,
      );
    }
  ...
  static createConfig(): AppConfig {
    try {
      ...
      const config: AppConfig = {
        ...
        OPENWEATHER_API_KEY: this.getOptionalEnv("OPENWEATHER_API_KEY"),
        DASHBOARD_CHANNEL_ID: this.getOptionalEnv("DASHBOARD_CHANNEL_ID"),
      };
      ...
      console.log(
        `   - Render URL: ${config.RENDER_URL ? "configured" : "not configured"}`,
      );
      console.log(
        `   - Dashboard: ${config.DASHBOARD_CHANNEL_ID ? "configured" : "not configured"}`,
      );
      console.log(
        `   - OpenWeather: ${config.OPENWEATHER_API_KEY ? "configured" : "not configured"}`,
      );
